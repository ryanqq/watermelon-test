...
public class AudioFFT {
	...
	final static double twopower15 = 32768.0;

	public static Complex[] FFT(byte[] rawByteData) {
		double[] rawDoubleData = getRawDoubleData(rawByteData);

		Complex[] fftArray = new Complex[rawDoubleData.length];
		for (int i = 0; i < rawDoubleData.length; i++)
			fftArray[i] = new Complex(rawDoubleData[i], 0);

		return fft(fftArray);
	}

	public static double[] getRawDoubleData(byte[] rawByteData) {
		...
		for (int i = 0, floatIndex = 0; i < bytesRecorded - bytesPerSample + 1; i += bytesPerSample, floatIndex++) {
			...
			double sample32 = amplification * (sample / twopower15);
	...
	public static void androidPrint(Complex[] x){
		Log.e("AudioRecorder", "------------------");
		for(int i = 0; i < x.length; i++)
			Log.e("AudioRecorder", x[i].toString());		
	}
}
